Shader "Unlit/liquido"
{
    Properties
    {
		...
		[Space(5)]
		_LiquidAmount ("Liquid Amount",Range(-6,6)) = 0
		_MidAmount ("Middle Amount",Range(0,0.5)) = 0
    }
    SubShader
    {
        ...
        Pass
        {
            ...
            v2f vert (appdata v)
            {
                ...
                o.uv = v.uv;
				float3 worldPosition = mul(unity_ObjectToWorld, v.vertex.yz);
				o.liquidEdge = worldPosition + _LiquidAmount;
                return o;
            }
			...
			float4 renderBase = _BaseColor + midEdgeColor;
			float4 renderTop = _TopColor * (midEdge + base);
